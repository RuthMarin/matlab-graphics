...
% Modified by Violeta Chang (July, 2013)
%
...
function aboxplot(position,X,varargin)
% position: 0 (below box-plot), 1 (above box-plot) --> indica dónde se
% coloca el  asteristo de pt-test
...
hgg = zeros(d,1);
ubicacion=zeros(n,2); %% 2 xq es ubicaciones (x,y)
%colors={'r','r','r';'b','b','b'};
%colors={'r','r','r';'w','w','w'};

for j=1:d %% d: num elementos por grupo
    % Get the j matrix
    if iscell(X)
        Y = X{j};
    else
...
%         hold off;
        ...
        if (position==1)
            if (ubicacion(i,1)==0)
                xMax=i-widthl/2+(2*j-1-widthe)*widthl/(2*d);
                xMin=i-widthl/2+(2*j-1+widthe)*widthl/(2*d);
                ubicacion(i,1)=(xMax + xMin)/2;
                ubicacion(i,2)=fu+0.05;
            else
                xActual=(i-widthl/2+(2*j-1-widthe)*widthl/(2*d) + i-widthl/2+(2*j-1+widthe)*widthl/(2*d))/2;

                yActual=fu+0.05;
                yActual=max(yActual,ubicacion(i,2));

                line([ubicacion(i,1) ubicacion(i,1)],[yActual yActual-0.01],'Color','k','HitTest','off','Parent',hgg(j));
                line([xActual xActual],[yActual yActual-0.01],'Color','k','HitTest','off','Parent',hgg(j));
                line([ubicacion(i,1) xActual],[yActual yActual],'Color','k','HitTest','off','Parent',hgg(j));

                xMedio=(ubicacion(i,1)+xActual)/2;
                text(xMedio,yActual+0.025,'*')
            end
        end
        ...
        if (position==0)
            if (ubicacion(i,1)==0)
                ...
                xMin=i-widthl/2+(2*j-1+widthe)*widthl/(2*d);
                ubicacion(i,1)=(xMax + xMin)/2;
                ubicacion(i,2)=fl-0.05;
            else
                xActual=(i-widthl/2+(2*j-1-widthe)*widthl/(2*d) + i-widthl/2+(2*j-1+widthe)*widthl/(2*d))/2;

                yActual=fl-0.05;
                yActual=min(yActual,ubicacion(i,2));
